...
export default function BubbleChart({ config, tables, filters, height = 300 }: Props) {
  const { option, hasData } = useMemo(() => {
    ...
    const sizeField = config.values[1];
    ...
    const data = rows.map(row => [
      parseFloat(String(row[xField.columnName] ?? 0)),
      parseFloat(String(row[yField.columnName] ?? 0)),
      parseFloat(String(row[sizeField.columnName] ?? 1))
    ]);
    ...
    return {
      ...
      option: {
        ...
        xAxis: { type: "value", name: config.xAxisLabel || "X Axis" },
        yAxis: { type: "value", name: config.yAxisLabel || "Y Axis" },
        series: [{
          name: sizeField.columnName,
          type: "scatter",
          symbolSize: (val: any) => Math.max(5, Math.min(40, val[2] / 10)),
          data,
